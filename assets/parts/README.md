...
**Files.** `base.svg` is the body and head. Every other file is one part, named `<slot>_<name>.svg` (`ears_`, `bow_`, `hair_`, `face_`, `outfit_`, `acc_`). The ids are fixed in D-44. An empty file is a stub (not drawn yet), except `*_none.svg`, which are empty on purpose.
...
- Node budget: 40 or fewer per character, ceiling about 50 (D-9). The rig is 18 of them. Merge shapes with the same fill into one `<path>` with several subpaths (for example both eyes).
